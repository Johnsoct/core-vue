...
const noReturnAssign = () => {
  ...
  const foo = (a, b) => this.value = a + b;
  function foo2(a, b) {
    return this.value = a + b;
  }

  const bar = (a, b) => (this.value = a + b);
  function bar2 (a, b) {
    return (this.value = a + b);
  }
...
const preferObjectSpread = () => {
  ...
  const meal = { ingredients, condiments, sides };
  // Technically, not error-prone, creates a copy (doesn't require deep here)
  // Pretty old syntax
  const meal2 = Object.assign({}, ingredients, condiments, sides);
